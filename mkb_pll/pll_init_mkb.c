/* This file contains API for PLL Initialization based on PLLTS12FFCFRACF for Mockingbird (MKB) */
 ...
 *
 * This function simulates PLL lock by checking the PLL_LOCKED_BIT in MKB_PLL_CTRL1_ADDR (0x26000100)
 *
 * @return - 1 if PLL is locked, 0 otherwise
 *
 */
static int check_pll_lock(void) {
	return (readl(MKB_PLL_CTRL1_ADDR) & PLL_LOCKED_BIT) ? 1 : 0;
}
...
pll_init_status_t pll_init_mkb(const float desired_output_freq_mhz[], uint32_t num_outputs, float input_ref_freq_mhz) {
	...
	// Program MKB_PLL_CTRL1 (0x26000100) (LOCK_BIT, DSM_EN_BIT, DAC_EN_BIT, ENABLE_BIT) 

	reg_ctrl1 = readl(MKB_PLL_CTRL1_ADDR); 
	writel(reg_ctrl1 & ~PLL_ENABLE_BIT, MKB_PLL_CTRL1_ADDR);

	// Program MKB_PLL_CTRL2 (0x26000104) (REFDIV, FBDIV_INT, POSDIV0, POSDIV1, POSDIV2)
	
	uint32_t fb_div_int = (uint32_t)pll_cfg.fb_div;

	reg_ctrl2 = readl(MKB_PLL_CTRL2_ADDR);
	...
	writel(reg_ctrl2, MKB_PLL_CTRL2_ADDR);

	// Program MKB_PLL_CTRL3 (0x26000108) (FRAC, POSDIV3, FOUT_EN)
	...
	reg_ctrl3 = readl(MKB_PLL_CTRL3_ADDR);
	...
	writel(reg_ctrl3, MKB_PLL_CTRL3_ADDR);

	// Program MKB_PLL_CTRL1 (0x26000100) (Fractional mode then enable PLL)
	...
	writel(reg_ctrl1, MKB_PLL_CTRL1_ADDR);
...
int main(void) {
	...
	pll_init_status_t status;
	
	/*** --- 1. Clock Generation of 133MHz for IPM PLL  --- ***/
	printf("\n *** --- Clock Generation of 133MHz for IPM PLL --- *** \n");
	printf("\n *** --- Input - 133MHz, Output - 1066MHz --- *** \n");
	float ipm_pll_output[] = {1066.0f};
	status = pll_init_mkb(ipm_pll_output, 1, 133.0f);
	if (status == PLL_INIT_SUCCESS) {
		printf("IPM PLL initialized and locked successfully\n");
		printf("Expected Output - 1066MHz\n");
	} else {
		printf("IPM PLL initialization and locked failed with error code: %d\n", status);
	}

	/*** --- 2. Clock Generation of 25MHz for MKB PLL --- ***/
	printf("\n *** --- Clock Generation of 25MHz for MKB PLL --- *** \n");
	printf("\n *** --- Input - 25MHz, Output - 1000MHz, 200MHz, 1000MHz, 1000MHz --- *** \n");
	float mkb_pll_output[] = {1000.0f, 200.0f, 1000.0f, 1000.0f};
	status = pll_init_mkb(mkb_pll_output, 4, 25.0f);
	if (status == PLL_INIT_SUCCESS) {
		printf("MKB PLL initialized and locked successfully\n");
		printf("Expected Output - 1000MHz, 200MHz, 1000MHz, 1000MHz\n");
	} else {
		printf("MKB PLL initialization and locked failed with error code: %d\n", status);
	}
	
	/*** --- 3. Clock Generation of 25MHz for GFH PLL --- ***/
	printf("\n *** --- Clock Generation of 25MHz for GFH PLL --- *** \n");
	printf("\n *** --- Input - 25MHz, Output - 1000MHz --- *** \n");
	float gfh_pll_output[] = {1000.0f};
	status = pll_init_mkb(gfh_pll_output, 1, 25.0f);
	if (status == PLL_INIT_SUCCESS) {
	...
	/*** --- 4. Clock Generation of 100MHz for PCIe / CXL RC and EP PHY (PCIe PLL) --- ***/
	printf("\n *** --- Clock Generation of 100MHz for PCIe / CXl RC and EP PHY (PCIe PLL) --- *** \n");
	printf("\n *** --- Input - 100MHz, Output - 1000MHz --- *** \n");
	float pcie_cxl_rc_ep_pll_output[] = {1000.0f};
	status = pll_init_mkb(pcie_cxl_rc_ep_pll_output, 1, 1000.0f);
	if (status == PLL_INIT_SUCCESS) {
		printf("PCIe / CXL RC and EP PHY (PCIe PLL) initialized and locked successfully\n");
		printf("Expected Output - 1000MHz\n");
	} else {
		printf("PCIe / CXL RC and EP PHY (PCIe PLL) initialization and locked failed with error code: %d\n", status);
	}
