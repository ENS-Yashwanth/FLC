/* This file contains test cases for the PLL Initialisation API based on PLLTS12FFCFRACF for Mockingbird */
...
void pll_frac_mode_lock(void) {
	...
	float output_freqs_mkb1[] = {1066.0f};
	pll_init_status_t status = pll_init_mkb(output_freqs_mkb1, 1, 133.0f);
	if (status == PLL_INIT_SUCCESS) {
		...
		while (timeout--) {
			uint32_t reg_val = readl(MKB_PLL_CTRL1_ADDR);
...
void pll_lock(void) {
	...
	float output_freqs_mkb2[] = {1000.0f};
	pll_init_status_t status = pll_init_mkb(output_freqs_mkb2, 1, 100.0f);
	if (status == PLL_INIT_SUCCESS) {
...
void pll_edge_case_lock_high_input(void) {
	...
	float output_freqs_mkb4[] = {1000.0f, 500.0f, 250.0f};
	pll_init_status_t status = pll_init_mkb(output_freqs_mkb4, 3, 500.0f);
	if (status == PLL_INIT_SUCCESS) {
...
void pll_edge_case_lock_low_input(void) {
	...
	float output_freqs_mkb5[] = {1000.0f};
	pll_init_status_t status = pll_init_mkb(output_freqs_mkb5, 1, 10.0f);
	if (status == PLL_INIT_SUCCESS) {
...
void pll_invalid_input_case_below_min(void) {
	...
	float output_freqs_mkb6[] = {100.0f};
	pll_init_status_t status = pll_init_mkb(output_freqs_mkb6, 1, 7.0f);
...
void pll_invalid_input_case_above_max(void) {
	...
	float output_freqs_mkb7[] = {1000.0f};
	pll_init_status_t status = pll_init_mkb(output_freqs_mkb7, 1, 700.0f);
	if (status == PLL_INVALID_PARAMS) {
...
void pll_invalid_output_case_below_min(void) {
	...
	float output_freqs_mkb8[] = {19.0f};
	pll_init_status_t status = pll_init_mkb(output_freqs_mkb8, 1, 100.0f);
...
void pll_invalid_output_case_above_max(void) {
	...
	float output_freqs_mkb9[] = {9000.0f};
	pll_init_status_t status = pll_init_mkb(output_freqs_mkb9, 1, 100.0f);
	if (status == PLL_FREQ_OUT_OF_RANGE) {
...
void pll_invalid_config_case_vco_range_mismatch(void) {
	...
	float output_freqs_mkb12[] = {50.0f};
	pll_init_status_t status = pll_init_mkb(output_freqs_mkb12, 1, 100.0f);
